...
exports.findAll = async (req, res, next) => {
  ...
  try {
    ...
    return next(new ApiError(500, 'An error occur while retrieving contacts'));
  }
};
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(
        500,
        `Error when retrieving contact with id = ${req.params.id}`
      )
    );
  }
};
...
exports.deleteAll = async (_, res, next) => {
  try {
    const contactService = new ContactService(MongoDB.client);
    ...
    console.log(error);
    return next(
      new ApiError(500, `An error occur while removing all contacts`)
    );
  }
};

exports.findAllFavorite = async (_, res, next) => {
  try {
    const contactService = new ContactService(MongoDB.client);
